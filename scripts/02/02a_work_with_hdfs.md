...
# damit auf jeden Fall die letztgültigen Dateien vorhanden sind.
cd ~/BigData/
git pull
...
# (führt man das "start" ohne vorheriges "stop" aus, gibt es Warning, dass halt einige schon laufen)
...
# optional sieht man Dateien/Ordner über "http://namenode:9870/explorer.html#/"
...
# WICHTIG: eventuell ist das ein Link, dann muss man auf das tatsächliche Verzeichnis schauen
...
# https://github.com/DistrictDataLabs/transportation-project-1/raw/master/airdelayhist/data/airline_delay_causes.csv
...
# Prüfen mit ganzen Verzeichnissen oder einzelnen Dateien
...
# entweder:
kill $(jps -v | grep datanode<IP-Address> | cut -d' ' -f1) # Kommando auf Datanode1 bzw. DataNode2
# oder:
hdfs --daemon stop datanode # Kommando, wenn man den primären Datanode auf NameNode stoppen will 
...
#                <value>10</value>
#        </property>
# einen zuvor gekillten Datanode (auf datanode1 oder datanode2) startet man danach am besten wieder direkt auf dem Datanode über folgende Befehlsfolge:
export SSH_CLIENT=<NameNode-IP-Address> # z.B. 10.77.16.124 (damit wir sicher unseren eigenen Datanode neu starten)
source ~/datanode.env
hdfs --daemon start datanode
# optional wenn datanode auf der primären VM (=namenode) gestoppt wurde, über
#    hdfs --daemon start datanode
